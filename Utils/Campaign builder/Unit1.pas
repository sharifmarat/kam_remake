...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure iMapMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure iMapMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure iMapMouseLeave(Sender: TObject);
...
procedure TForm1.aAddChapterExecute(Sender: TObject);
var
  Node: TTreeNode;
begin
...
procedure TForm1.iMapMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  FMouseMove := Button = mbLeft;
  if FMouseMove then
    Screen.Cursor := crHandMove;

  FMousePosition := Point(X - ScrollBox1.HorzScrollBar.Position, Y - ScrollBox1.VertScrollBar.Position);
end;

procedure TForm1.iMapMouseLeave(Sender: TObject);
begin
  FMouseMove := False;
  Screen.Cursor := crDefault;
end;

procedure TForm1.iMapMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  Position: TPoint;
begin
  if FMouseMove then
  begin
    Position := Point(X - ScrollBox1.HorzScrollBar.Position, Y - ScrollBox1.VertScrollBar.Position);
    ScrollBox1.HorzScrollBar.Position := ScrollBox1.HorzScrollBar.Position + (FMousePosition.X - Position.X);
    ScrollBox1.VertScrollBar.Position := ScrollBox1.VertScrollBar.Position + (FMousePosition.Y - Position.Y);

    FMousePosition := Position;
  end;
end;

procedure TForm1.iMapMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  FMouseMove := False;
...
procedure TForm1.pmMapPopup(Sender: TObject);
begin
...
procedure TForm1.AddMission(X, Y: Integer);
begin
  if fUpdating then
    Exit;
   {
...
procedure TForm1.aAddMissionExecute(Sender: TObject);
var
  Node: TTreeNode;
begin
...
procedure TForm1.aAddNodeExecute(Sender: TObject);
var
  curItem: Integer;
begin
  if fUpdating then
...
procedure TForm1.aOpenPictureExecute(Sender: TObject);
begin
...
begin
  Caption := 'Campaign Builder (' + GAME_REVISION + ') ' + StringReplace(C.CampName, #0, '_', [rfReplaceAll, rfIgnoreCase]) + ' - ' + C.FullName;
end;

procedure TForm1.CampaignToList;
var
  I, K, J: Integer;
  N, SN: TTreeNode;
...
procedure TForm1.ListToCampaign;
var
  i, k: Integer;
  ChapterCount: Byte;
  node: TTreeNode;
...
procedure TForm1.tvListCreateNodeClass(Sender: TCustomTreeView; var NodeClass: TTreeNodeClass);
begin
  NodeClass := FTreeItemCreate;
end;

procedure TForm1.tvListDragDrop(Sender, Source: TObject; X, Y: Integer);
var
  AnItem: TTreeNode;
...
procedure TForm1.tvListDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
var
